...
ifneq ($(wildcard $(EXTRACTED_DIR)),)
  ...
  DRAGONEW_PNG_FILES := $(shell find $(EXTRACTED_DIR) -name '*.png')
  DRAGONEW_csdata_f32_FILES := $(shell find $(EXTRACTED_DIR) -name '*.csdata.f32.inc.c')
else
  DRAGONEW_BIN_FILES :=
  DRAGONEW_PNG_FILES :=
  DRAGONEW_csdata_f32_FILES :=
endif
...
$(ELF): $(DRAGONEW_O_FILES) $(DRAGONEW_INC_C_FILES) $(TEXTURE_FILES_OUT) $(ASSET_FILES_OUT) $(O_FILES) $(OVL_RELOC_FILES) $(LDSCRIPT) $(BUILD_DIR)/undefined_syms.txt
	$(LD) -T $(LDSCRIPT) -T $(BUILD_DIR)/undefined_syms.txt --no-check-sections --accept-unknown-input-arch --emit-relocs -Map $(MAP) -o $@

$(BUILD_DIR)/%.u8.inc.c: %.u8.bin
	@echo // From file://`realpath $<` >$@
	tools/assets/bin2c/bin2c u8 <$< >>$@

$(BUILD_DIR)/%.u32.inc.c: %.u32.bin
	@echo // From file://`realpath $<` >$@
	tools/assets/bin2c/bin2c u32 <$< >>$@

$(BUILD_DIR)/%.u64.inc.c: %.u64.bin
	@echo // From file://`realpath $<` >$@
