...
def main():
    ...
    for file, bss_section in bss_sections.items():
        if not bss_section.pointers:
            continue
        # The following heuristic doesn't work for some files, since the first pointer into BSS is not
        # at the start of the section. Fortunately these files have only one BSS variable so we can
        # just skip them.
        if str(file) in ("src/boot/z_locale.c", "src/libleo/leo/leointerrupt.c"):
            continue
