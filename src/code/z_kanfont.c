...
void Font_LoadOrderedFont(Font* font) {
    ...
    DMAMGR_REQUESTSYNC(font->msgBuf, (uintptr_t)_nes_message_data_staticSegmentRomStart + font->msgOffset, len,
                            "../z_kanfont.c", 122);
    ...
    for (fontBufIndex = 0, codePointIndex = 0; font->msgBuf[codePointIndex] != MESSAGE_END; codePointIndex++) {
        ...
        if (font->msgBuf[codePointIndex] != MESSAGE_NEWLINE) {
            ...
            DMAMGR_REQUESTSYNC(fontBuf, fontStatic + offset, FONT_CHAR_TEX_SIZE, "../z_kanfont.c", 134);
