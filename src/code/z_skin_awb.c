...
void Skin_Init(PlayState* play, Skin* skin, SkeletonHeader* skeletonHeader, AnimationHeader* animationHeader) {
    ...
    skin->vtxTable = ZELDAARENA_MALLOC(limbCount * sizeof(SkinLimbVtx), "../z_skin_awb.c", 212);
    ...
    for (i = 0; i < limbCount; i++) {
        ...
        if ((limb->segmentType != SKIN_LIMB_TYPE_ANIMATED) || (limb->segment == NULL)) {
            ...
            vtxEntry->buf[0] =
                ZELDAARENA_MALLOC(animatedLimbData->totalVtxCount * sizeof(Vtx), "../z_skin_awb.c", 235);
            ASSERT(vtxEntry->buf[0] != NULL, "psavb->buf[0] != NULL", "../z_skin_awb.c", 237);

            vtxEntry->buf[1] =
                ZELDAARENA_MALLOC(animatedLimbData->totalVtxCount * sizeof(Vtx), "../z_skin_awb.c", 240);
    ...
    if (skin->vtxTable != NULL) {
        ...
        for (i = 0; i < skin->limbCount; i++) {
            if (skin->vtxTable[i].buf[0] != NULL) {
                ZELDAARENA_FREE(skin->vtxTable[i].buf[0], "../z_skin_awb.c", 276);
                skin->vtxTable[i].buf[0] = NULL;
            }
            if (skin->vtxTable[i].buf[1] != NULL) {
                ZELDAARENA_FREE(skin->vtxTable[i].buf[1], "../z_skin_awb.c", 280);
                skin->vtxTable[i].buf[1] = NULL;
            }
        }

        if (skin->vtxTable != NULL) {
            ZELDAARENA_FREE(skin->vtxTable, "../z_skin_awb.c", 286);
        }
